...
${\rm GM}(\lambda) < {\rm AM}(\lambda)$를 만족하는 $\lambda$를 defective eigenvalue라고 부른다. 특성 방정식에서 해당 아이겐밸류가 생성하는 널 스페이스의 차원이 AM보다 작다면, 아이겐벡터를 모아서 특성 방정식의 널 스페이스를 생성할 수 없다. 이는 곧 아이겐 분해를 통해서 원래 매트릭스의 컬럼 스페이스를 생성할 수 없다는 의미다.  
