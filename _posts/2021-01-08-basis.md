...
$$
(a_1, \dotsc, a_n)_{B_{\hat e}} = \underbrace{(\vec a \cdot \hat e_i)}_{a_1} \hat e_i1 + \dotsb + (\vec a \cdot \hat e_n) \hat e_n
$$
...
$$
\{ \vec v_1 = (v_{1x}, v_{2x})^T, \vec v_2 = (v_{2x}, v_{2y})^T\}
$$
...
1. $C^{-1} x$는 $[x]_{u}$의 기저를 $[x]_{\mathcal B}$로 바꾸는 것이다. 즉, $\phantom{}_{\mathcal B}{1}_{u}$
2. 이 바뀐 기저에서 $B$이라는 변환을 수행한다. 즉, $\phantom{}_{\mathcal B}{B}_{\mathcal B}$
3. $C$는 곱해 다시 통상적인 기저로 돌아오게 된다. 즉,  $\phantom{}_{u}{1}_{\mathcal B}$

아래 그림에서 보듯이, $Ax$라는 변환과 $B[x]_{\mathcal B}$라는 변환은 좌표계만 다를 뿐 동일한 변환이다. $C B C^{-1}$의 기저의 변화를 살펴보면, $u \rightarrow \mathcal B \rightarrow B \rightarrow u$로 나타낼 수 있다.  

![enter image description here](https://github.com/anarinsk/lostineconomics-v2-1/blob/master/images/basis/coord.png?raw=true){: style="margin: auto; display: block; border:1.5px solid #021a40;"}{: width="500"}
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTkyNTIwNzg3MiwtMTY3NjY1NDgzOSwtMT
IyNDI4NzQxOSw5NjczNzcwLC0zNjE0NDg0NTUsLTE2MDE3OTUw
OTksLTE5NDcxMjQ2NjMsMTU0MTUyNTQxMSwtMTIyMzEzMTkxNy
wxMzY4Mjk4MDcyLC01MzI0NTE1MDMsLTg3MDQ5MjI0OSwxMjE0
MDgxNzc0LDE3ODQyODUzMjEsNzc3NTA4MDEwLDExNTIyOTYyNz
YsLTEzMjU0MjUwNDIsMTM0NzMzOTAxMSw5ODk1NDU4OTAsLTEz
ODU2MDY2MzZdfQ==
-->