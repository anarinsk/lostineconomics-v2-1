...
$M$과 $N$은 서로 닮은 꼴의 매트릭스다. 위 식을 만족하는 매트릭스 $M$과 $N$을 similar matrix라고 정의한다. 우선 두 매트릭스가 서로 닮은 꼴일 때에는 $n \geq 1$에 대해서 $M^n = C N^n C^{-1}$이 성립한다. 이는 아이겐 분해에서 보듯이 $N$이 어떤 매트릭스냐에 따라서 매우 계산 상 편리함을 준다. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbOTc2MzE1NzM5LDIzOTYxMzg1MCwtODMyOD
IwODZdfQ==
-->