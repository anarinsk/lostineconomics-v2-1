...
우도(likelihood)는 무엇일까? 우도는 확률 분포의 수학적인 형태를 뒤집은 것이다. 즉 어떤 샘플이 관찰되었다고 할 때 분포의 특정 파라미터 하에서 얻게 되는 확률 밀도함수를 의미한다. 연속 확률변수가 아니라 이산 확률변수라면, 확률 질량함수가 될 것이고 이 경우에는 확률과 같다. 요컨대 목표에 따라서 주어진 것(conditioning)의 차이에 따라서 확률과 우도의 정의가 달라진다. 다시 정리해보자. 우도를 일종의 함수라고 할 때 그 함수의 $x$에 해당하는 것이 파라미터다. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjAzNjQxMTk0NSwxNzIwMjYxMDU3LC05Nj
