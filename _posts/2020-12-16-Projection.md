...
어차피 벡터는 점과 달리 상대적인 위치로 정의된다. $\Pi_{P^\perp}$를 이동해보자. 그러면 벡터 프로젝션이 자연스럽다. 즉,  $\Pi_{P^\perp}$로 스칼라 프로젝션을 한 후 표준화된 벡터에 이를 곱해주면 된다. 이제 $\Pi_{P^\perp}$ 대입하면 $\Pi_{P}$를 쉽게 구할 수 있다. 즉, 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTMzMjc0MTY1MSwxODAzMzU1Mzk0LDEzOT
