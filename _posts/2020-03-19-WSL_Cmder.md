...
WSL의 또다른 장점은 리눅스 내에서 윈도 어플리케이션이 실행된다는 것이다. 예를 들어, `wsl.exe -l -v`로 WSL 버전을 확인할 수도 있고, `code .`을 치면 VS Code가 실행된다. `code `
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTI0MjE2MzkwOCwtMzMxMDQ2MDIwLDEwNj
