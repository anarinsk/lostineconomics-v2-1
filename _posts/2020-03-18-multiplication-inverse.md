...
- ${\mathbf A} x = {\mathbf 0}$를 만족하는 $\mathbf 0$ 벡터가 아닌 벡터 $x$가 존재하면 singular matrix. 
- 증명은 간단하다. 
	- $\mathbf A$의 역행렬이 존재하고, $x \neq 0$라고 하자.
	- ${\mathbf A}^{-1} {\mathbf A} x = {\mathbf A}^{-1} {\mathbf 0} = {\mathbf 0}$
	- 따라서 $Ix = {\mathbf 0}$이 되고, $x \neq {\math0$와 모순이다.  
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIwODg1Mjk1NjUsMjE3NTUwOTA0XX0=
-->