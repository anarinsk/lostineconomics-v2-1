...
위 두 명령은 각각 `<Distro>`를 내리고 등록을 취소하라는 것이다. 그리고 윈도에 설치한 Ubuntu를 실행하면 우분투 배포판의 설정을 처음부터 다시 진행한다. ID와 password를 등록하면 된다. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbNjQyMTczNTc5LDEzNTIxMzk2ODldfQ==
-->