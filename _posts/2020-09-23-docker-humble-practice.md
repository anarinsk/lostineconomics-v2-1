...
내가 쓰는 세팅을 잘 설정해서 특화된 이미지로 특정 시점에 말아두었다고 하자. 당장 편리하게 끌어다 쓸 수 있을 것이다. 하지만 이렇게 고정된 이미지를 쓸 경우  도커 이미지 안에 속한 패키지나 구성요소들을 별도로 업데이트 해줘야 한다. 시간이 지날수록 업데이트해야 할 항목과 양이 늘어나게 된다. 한편 공식 이미지의 경우 필요한 업데이트를 이미지 관리자가 정기적으로 수행하기 때문에 이미지를 한번 끌어오는 것으로 충분하다. 

팀별로 통일된 환경이 필요하고 이 환경을 조직에서 관리할 수 있다면, 특화된 이미지를 만들어두고 쓰는 편ㅇ다. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTExOTE3NzE4NTUsMzgwODk3MTk3LC0xOD
